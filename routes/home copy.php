...
use Jiny\Chat\Http\Controllers\Home\Room\DeleteController;
...
use Jiny\Chat\Http\Controllers\Home\Server\ServerShowController;
use Jiny\Chat\Http\Controllers\Home\Server\MessageController as ServerMessageController;
use Jiny\Chat\Http\Controllers\Home\Server\MessageSendController as ServerMessageSendController;
use Jiny\Chat\Http\Controllers\Home\Server\SseController;
use Jiny\Chat\Http\Controllers\Home\Server\SseParticipantsController;
use Jiny\Chat\Http\Controllers\Home\Server\SseMessageSendController;
use Jiny\Chat\Http\Controllers\Home\Server\SseStreamController;
use Jiny\Chat\Http\Controllers\Home\Server\SseHeartbeatController;
...
Route::middleware(['web', 'jwt'])->prefix('home/chat')->name('home.chat.')->group(function () {
    ...
    // 채팅방 삭제 - SAC
    Route::delete('/room/{id}', DeleteController::class)->name('room.delete');


    // 서버형 채팅방 라우트 (SSE 방식) - SAC
    Route::get('/server/{id}', ServerShowController::class)->name('server');

    // SSE 실시간 채팅방 라우트 - SAC
    Route::get('/server/sse/{roomId}', SseController::class)->name('server.sse');
    ...
    Route::prefix('api')->name('api.')->group(function () {
        ...
        Route::prefix('server')->name('server.')->group(function () {
            // 서버 채팅방 메시지 목록 조회 - SAC
            Route::get('/{id}/messages', ServerMessageController::class)->name('messages');

            // 서버 채팅방 메시지 전송 - SAC
            Route::post('/{id}/message', ServerMessageSendController::class)->name('message.send');

            // SSE 채팅방 API - SAC
            Route::prefix('sse')->name('sse.')->group(function () {
                // SSE 이벤트 스트림
                Route::get('/{roomId}/stream', SseStreamController::class)->name('stream');

                // SSE 참여자 목록 조회
                Route::get('/{roomId}/participants', SseParticipantsController::class)->name('participants');

                // SSE 메시지 전송
                Route::post('/{roomId}/message', SseMessageSendController::class)->name('message.send');

                // SSE 하트비트
                Route::post('/{roomId}/heartbeat', SseHeartbeatController::class)->name('heartbeat');
            });
        });
    });
    ...
    Route::delete('/rooms/{id}/delete-invite', InviteDeleteController::class)->name('rooms.delete-invite');
